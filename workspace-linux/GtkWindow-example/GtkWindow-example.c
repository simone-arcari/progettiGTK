...
#define BUTTON_LABEL_FC    "Fullscreen"
#define BUTTON_LABEL_UFC    "Unfullscreen"
...
static void activate(GtkApplication *app, gpointer user_data) {
    ...
    /* button */
    button = gtk_button_new_with_label(BUTTON_LABEL_FC);
    g_signal_connect(button, "clicked", G_CALLBACK(fullscreen_switch), window);
...
static void fullscreen_switch(GtkWidget *button, gpointer window) {

    if(gtk_window_is_fullscreen(GTK_WINDOW(window))) {

        gtk_window_unfullscreen(GTK_WINDOW(window));
        gtk_button_set_label(GTK_BUTTON(button), BUTTON_LABEL_FC);
    }else {

        gtk_window_fullscreen(GTK_WINDOW(window));
        gtk_button_set_label(GTK_BUTTON(button), BUTTON_LABEL_UFC);
    }
}   